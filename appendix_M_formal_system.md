...
**CORRECTED STATEMENT (Theorem 14).** The conclusion above is false as quantified, and the statement preserved unrevised above is kept per the retention convention so the correction can be read against it. What the proof sketch actually establishes is:

$$\liminf_{t \to \infty} \Pr[\Psi_t \in \text{basin}(\Psi^*)] \ \geq\ \rho_\infty(\text{basin}) \quad\text{and}\quad \rho_\infty(\text{basin}) \to 1 \ \text{ as } \ \Delta P / D \to \infty.$$

Two amendments are required and neither is cosmetic. **(a)** $\epsilon$ cannot range over all positive values at fixed noise; it is bounded below by the stationary mass outside the basin. The unqualified "for any $\epsilon > 0$" is available only in the limit $\Delta P / D \to \infty$, which is where the theorem's force actually lies. **(b)** If the pointwise-in-$t$ form is kept, hypothesis (ii) must be strengthened from ergodicity to **mixing**: Birkhoff gives time averages, and the conclusion is about marginals.

**Implication, as corrected**: this remains RE's strongest necessity result, but its content is a *limit* claim rather than an unconditional one. Above the threshold, the carrier-class recipe is occupied with probability bounded below by its stationary mass, and that mass approaches certainty as the basin deepens relative to the noise. "Inevitable" is correct in the low-noise limit and an over-claim outside it. This still strengthens Conjecture 4 from possibility to quantified convergence, and still grounds the "trajectory convergent" claim of ch3 §3.6 in established theorems — but the grounding is asymptotic, and the earlier phrasing asserted more than Morse, Birkhoff and Kramers deliver.

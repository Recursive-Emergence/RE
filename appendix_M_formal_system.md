...
- **The price of honest measures.** Three times now, replacing a stand-in with a genuine measure or law over arbitrary predicates has brought classical choice in with it: the marginal laws of the kernel representation, the witnesses of the pushforward probe, and the legal witness behind Theorem 10. The reason is the same each time — a genuine measure must decide whether an arbitrary predicate holds at a point, which is not constructive — and so is its limit: the *general* statements about such measures stay choice-free, as the pushforward's additivity does. Choice enters where a particular measure is evaluated, not where measures are reasoned about.
...
**Boundary, as with the others**: Theorem 7 is untouched. Morse theory, Birkhoff and Kramers are not in question, and neither is the manuscript's proof sketch. What failed is the transcription, for the third time in three attempts.

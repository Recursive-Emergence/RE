...
- **Exactness is a declared hypothesis, and a strong one.** The skeleton computes laws in integers at scale 100, which truncates for most kernels. General statements therefore assume the kernel exact at that scale; on a discrete grid this is close to requiring that the chain reach stationarity in finitely many steps. No witness in the record is excluded by it.

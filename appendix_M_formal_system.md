...
*On the verification apparatus in this appendix — pre-registered experiments, declared destinations, machine-checked audits, `formal/BoundaryResidue.lean` and its `check.sh` — a word about scope, since apparatus without stated scope reads as ceremony. It exists to police one boundary: the one between this framework's informal register and its formal one. That boundary is where the errors have actually been. Thirteen are on record, and they are one mode — the informal register borrowing the authority of a formal operation it has not paid for. Definition 16's set membership was really an obligation to exhibit a decode. Problem 21's "an encoding" was really an obligation to exhibit a provable round trip available to the layer. The shared letter $P$ across Theorem 7 and Definition 1 was really an obligation to state an aggregation over fibers. Problem 26's "restriction to the layer's vocabulary" named an operation this framework does not define. A destination clause filed two theorems into the conjectures section. And two transpositions into the formal frame — of Theorem 8 (i) and of Conjecture 7 (b) — asserted that they carried their truth with them, which they did not (Corollaries 10.1, 10.2); a transposition is prose asserting truth-preservation, and nobody had checked it, through two reviews of those very statements. The first three are the manuscript's older strata; the remaining four were written in the audit that named the mode, by the reviewer applying it and by the author implementing it. And the same two transpositions failed a second time, differently: the opacity one had inverted Razborov–Rudich's logical shape, asserting *large $\Rightarrow$ not constructive* where the theorem concludes *constructive $\wedge$ large $\Rightarrow$ not useful*, and the diagonal one had replaced self-application with identity-with-a-point. And the transcription of Theorem 7 dropped five ingredients at once, normalization among them, which made it false (Theorem 13). Counted as ten. Three transcriptions attempted, three false; every one of them survived multiple readings by both of us before a machine was aimed at it. And the general Parts A and B of Theorem 7's replacement, as first stated on finite spaces, dropped the hypothesis that the kernel is a kernel — no field made its rows probability measures — and were false as stated, refuted by the reviewer before the conjecture marker on them was a day old. The hypothesis list had come from a merge condition and was transcribed as given; neither author nor reviewer noticed what it omitted. Counted as eleven. And the joined signature built for Problem 26's probe transcribed A2's surjective, many-to-one *function* as a bare relation — dropping single-valuedness, totality and surjectivity together — and its only witness related every state to every structure. The author wrote it; it was found by the first check of Problem 25's pushforward probe, before the pushforward was defined. Counted as twelve. And the structure carrying Theorem 7's stationary measure required only that the basin have positive mass: nothing gave the empty set mass zero, nothing made masses add over disjoint sets, and one witness gave every set mass one. "The stationary measure is a measure" was a gloss, as "it is the stationary law" had been in Theorem 7's first draft. Distinct from the normalization loss of Theorem 13 and found a different way — not by aiming a theorem at a statement, but by accident, while auditing a witness for another property. Counted as thirteen. The apparatus is not a claim of rigor — it is the acknowledgement that this particular boundary leaks in both directions, and the record shows it catching errors on both sides. The leak has a uniform direction: every one of the thirteen entries is a loss in transport — a dropped conjunct, quantifier, constraint, normalization or link — and nothing in the record was ever fabricated. The failure mode is subtractive, which is why the countermeasure in Open Problem 29 is an enumeration checked off rather than a re-reading. That countermeasure has since run once, on Theorem 7, and produced a finding of a **second kind**: the transcription was faithful and the theorem was false (Theorem 14). The ledger counts transport losses and now stands at thirteen; over-claims by the informal register are a separate category, opened by Theorem 14, and the apparatus is built to find both. **That category has two entries, and the second was written by the batch that opened it**: the first proposed replacement for Theorem 7 asserted a limit that fails for local maxima, in prose, without being put through the loop it had just demonstrated. The pattern was applied one step too few, immediately after being named. The lesson is recorded here rather than in the loop's own conclusion, because a countermeasure that its authors forget to run on their own repair is not yet a habit.*
...
*Formal record, 2026-09-10:* the first theorems about this potential in the machine-checked record are the bridging-law witnesses of Problem 26. Before them, no theorem in the record had been about it at all.
...
$$\lim_{t \to \infty} \frac{1}{t}\sum_{s < t} \mathbf{1}[\Psi_s \in \text{basin}(\Psi^*)] \ =\ \rho_\infty(\text{basin}(\Psi^*)).$$

The limit exists — not a $\liminf$ bound. The almost-sure scope is over the noise, per $\Psi_0$; no uniformity in $\Psi_0$ is claimed. **Stated in discrete time** (amendment A9). For the SDE of Part C it applies to the time-1 skeleton chain, which is positive Harris recurrent when the diffusion is, and whose limit is the same $\rho_\infty(\text{basin})$.

**Which form the formal record holds.** The almost-sure statement lives on path space, and the skeleton's signature carries no path measure. The formal record therefore holds the **Cesàro-marginal** form, $\lim_{t\to\infty} \frac{1}{t}\sum_{s<t} \Pr_{\Psi_0}[\Psi_s \in \text{basin}(\Psi^*)] = \rho_\infty(\text{basin}(\Psi^*))$ — a theorem under positive Harris recurrence, hostable with marginal laws alone, and **strictly weaker** than the display above. The display states the stronger claim; the machine-checked term states the weaker one; this sentence is the connection, so the two cannot be read as agreeing.
...
each positive Harris recurrent with invariant probability measure $\rho_\infty^{(D)}$. Only these process hypotheses are inherited from Part A; no local maximum is distinguished in the proposition, and one is first singled out in the corollary.
...
**And the consequence to record when a fixed version lands**: with "global" added, recipe inevitability becomes a *sharper* claim than the original, not a weaker one — at low noise a substrate converges on the basins of the **global maxima** of $P$, and merely local recipes are transient. That is a prediction the original statement never made, and it is testable in a way "inevitable" was not.

**Implication, as written against the reviewer's specification, enumerated against it, and amended A1–A9 (2026-09-10)**: this remains RE's strongest necessity result, but its content is now three claims of different strengths rather than one. Parts A and B say only that a basin is occupied at its stationary mass — a *quantified* statement with no inevitability in it at any noise level. Part C is where necessity lives. Under Part C's hypotheses, at low noise, what it makes inevitable is the **union of the global-maximum basins**, not any one of them: a substrate is found in the basin of some global maximum with probability approaching one, while every merely-local basin empties. Only under the corollary's extra hypothesis — that the global maximum is unique — does this become a claim about *the* recipe in the singular. Where several carrier recipes share the global maximum value the framework predicts a split, not a winner, and the original phrasing had no way to say that. This still strengthens Conjecture 4 from possibility to quantified convergence and still grounds the "trajectory convergent" claim of ch3 §3.6 — but the grounding is asymptotic in the noise, it concerns a union rather than a point, and the original asserted more than Morse, Birkhoff and Kramers deliver.

**Formal-record notes for the replacement** — kept here rather than only in the skeleton's comments, since this is where a reader of the manuscript will look.

- **The kernel (item 16).** The skeleton's original signature carried only *marginal* laws, and Part A cannot be stated against marginals at all: positive Harris recurrence is a property of a transition kernel, two kernels can share every marginal, and "almost surely" lives on path space. A one-step kernel was added, with laws *derived* by iteration. The old counterexample of Theorem 14 turned out to be derivable from such a kernel — it was a Markov chain that had not been told so — and $\rho_\infty$ acquired its first definition in the record as a formula, invariance under the kernel, rather than a gloss.
- **A1 bites only on infinite spaces.** On a finite state space, a unique invariant probability forces a single recurrent class that every start reaches, so "unique" and "positive Harris" coincide there and the strengthening changes nothing. The refuting witness for the unamended Part A is necessarily infinite, and no finite chain could have served.
- **Harris is declared, not proved.** In the formal record, strict positivity of the kernel stands in for positive Harris recurrence. On a finite space strict positivity implies it — the standard fact — but it is strictly stronger, so the record's general Parts A and B cover fewer chains than the displays do.
- **Exactness is a declared hypothesis, and a strong one.** The skeleton computes laws in integers at scale 100, which truncates for most kernels. General statements therefore assume the kernel exact at that scale; on a discrete grid this is close to requiring that the chain reach stationarity in finitely many steps. No witness in the record is excluded by it. *Sharpened 2026-09-10 (target (xiii)):* together with the other hypotheses it does require it. Every chain in the class is exactly stationary from step 100 on, from any start — machine-checked (`dob_stationary`).
- **Two identical-looking terms that were not the same object.** Refuting the unamended Part A, two conditional expressions that print identically failed to unify: one carried a classical decidability instance, inherited from an arbitrary predicate, and the other a genuine one, acquired after instantiation. It is the failure mode the front matter names — same surface text, different objects — found one level down, *inside* the formal register, and caught by the compiler rather than by reading. **It recurred twice more before this replacement was done, and three is a pattern worth naming.** A pre-commit guard, searching for a declaration by name, matched a *different* declaration whose name began with the same letters, and reported a false duplicate; that one was in the verification tooling, not the proofs. And a list defined as an ordinary definition failed to match its own unfolded contents in a rewrite, where a reducible abbreviation matched at once. Three instances — in proof terms, in tooling, in definitions — of text that reads identically standing for things the machine treats as different. In each case the compiler or a direct query caught it, and reading did not. A fourth, again in tooling: a range meant to bound one declaration's signature was ended by an indentation pattern that never matched, so it ran to the end of the file and reported nine false occurrences; the signature itself had none. A fifth, and the first caught before anything relied on it: a dependency enumeration done by pattern-matching the source returned eighty-five dependents where the real number was fourteen, because three one-letter definitions matched every variable of the same name. **Since then the practice is fixed: enumerations over the formal record are done on its terms, by the proof assistant, not on its text.** A sixth, inside the fixed practice itself: an enumeration done by the proof assistant, of what depends on the layer's cost field, looked the field up by its bare name while the frame's names carry a namespace, and reported that nothing depended on cost at all. A positive control caught it before the number was used — a definition built literally out of the cost field had to appear among its dependents, and did not. Moving an enumeration from the text to the terms takes the text out of the *answer*, not out of the *question*; such enumerations now carry a positive control. *Order of operations.* Separate from the fourth instance, and more serious, is the order in which it happened — the claim that check was meant to support had already been committed. **A claim committed before its check is a broken guard even when the claim turns out true.** It is the contract-break that the rule against unmarked unproved statements describes, one level up. A second instance came during Problem 26's probe, in two forms at once: a commit chain let a failed edit step pass and committed a message naming changes the commit did not contain, and the diagnosis of that failure named the wrong step before counting which one had failed. Both were caught before anything left the working branch; the chain now aborts on any failed step.
- **What is and is not machine-checked.** Parts A and B are stated generally in the record on finite spaces, as conjectures with two proved instances beside them: the two-state chain they were built around, and a three-state chain with unequal rows built as an independent check. The second is *one instance, finite-horizon by construction* — it reaches its stationary law exactly at the second step, so it exercises the hypotheses and not the asymptotic mixing that makes the general statements hard; the generic finite case remains unproved. *Resolved 2026-09-10 (target (xiii)):* the generic finite case is now proved on the record's class, with no hypothesis added. That class forces exact stationarity within 100 steps, so the proof, too, rests on discreteness rather than asymptotic mixing: the difficulty just named is avoided, not overcome. Exactness at single states turns out to suffice, and invariance, though stated with truncating division, turns out to be exact (`invariance_exact`). The statements nonetheless keep exactness at every set, a hypothesis strictly stronger than the proof uses, for stability of what was pre-registered; restating them is a one-line change with a re-run, logged for later. This is their second form; the first was false (next bullet). **Part C has no Lean marker**: its general statement needs $\mathbb{R}^n$, which the Mathlib-free skeleton cannot host, and the record holds only the refutation of its predecessor and its finite confirmations. The "not machine-checked" sentence at Part C is its marker.

- **The kernel was not required to be a kernel.** The first general statements of Parts A and B listed exactness, strict positivity and invariance, and no hypothesis that each row of the kernel is a probability measure. A positive, exact kernel with unequal row totals — one row summing to one half, the other to three halves — admits an invariant measure and satisfies every listed hypothesis, yet its laws never converge, and it refutes both statements. Refuted by the reviewer before the conjecture marker was a day old. The amendment requires each row to be stochastic and additive. **This is not a weakening of the display**: a positive Harris process is a Markov process, whose kernel is stochastic by definition. It is the hypothesis the transcription had dropped. **Both new conditions are forced, each by its own counterexample.** The first refuting kernel fails stochasticity but is additive. A second, built by the reviewer, is stochastic but not additive — its rows total the right amount as a whole while their individual points do not add up to it — and it refutes both statements too. So stochasticity alone is not enough, and neither condition could have been left out. An earlier version of this bullet said additivity rested on argument alone; that was true when written, and the second counterexample superseded it.

- **A lemma for witness-legality checks.** When a witness turns out to be one the manuscript rules out, restricting to legal witnesses can only narrow the class a result ranges over. Every result stated for *all* of them survives; only results that got their force from *exhibiting* one — existence claims, and refutations of universal claims — can move. A legality check therefore needs the dependency closure of the illegal witness, filtered to those two kinds, and nothing more. First used for the re-check behind Theorem 10, where recall had named three sections that could not move and the closure named the one that could.
- **The price of honest measures.** Four times now, replacing a stand-in with a genuine measure or law over arbitrary predicates has brought classical choice in with it: the marginal laws of the kernel representation, the witnesses of the pushforward probe, the legal witness behind Theorem 10, and the repair of the two witnesses whose stationary "measure" had not been one. The reason is the same each time — a genuine measure must decide whether an arbitrary predicate holds at a point, which is not constructive — and so is its limit: the *general* statements about such measures stay choice-free, as the pushforward's additivity does. Choice enters where a particular measure is evaluated, not where measures are reasoned about.

**State of the record at this merge.** Theorem 7 as originally stated is refuted, and its first formal transcription is refuted and gone. It is replaced by three separately hypothesized displays. Of these, Parts A and B are stated generally on finite spaces as two conjectures, each with its unamended predecessor refuted in the same file, each added hypothesis forced by its own counterexample, and two proved instances — a two-state chain and an independently built three-state one. Part C's general claim is marked not machine-checked, with the reasons. The formal record carries exactly two unproved statements, and they are the first in its history to have been attacked before they were committed rather than after; they survived that attack and one more that came later. That is not the same as being proved, and the record has twice learned the difference the hard way. *Updated 2026-09-10 (target (xiii)):* both are now proved, and the formal record carries no unproved statement. That means Parts A and B hold for finite, strictly positive kernels exact at scale 100 — and no more: not Doeblin's theorem, nothing about kernels that truncate, and neither the declared Harris condition nor declared exactness discharged. Every commit on the way here was reproduced independently, by the reviewer, five times in all.
...
**Transcription status**: the first formal transcription was false as written and was demoted (Theorem 13; Open Problem 29). The *second* was written against an enumeration committed in advance and is faithful — and it refutes the theorem (Theorem 14). See the corrected statement below. The theorem itself is unaffected — what failed was the transport of its statement into the skeleton, which dropped normalization, the initial-condition universal, the $\theta$-parameter and its threshold, the basin's link to $P$, and the ergodicity hypothesis itself.
...
| Requires | Substrate above $\theta_c$, time, **and low noise relative to basin depth** (Theorem 14) | Self-representing depth |
| Guarantees | Reachability | Novelty |
| Fails to give | Anything not already on the manifold | Which extension survives |

**The division is one of signature, not of degree** (verified: `formal/BoundaryResidue.lean` §6). Theorem 7 transcribes and compiles with no reference to the representation apparatus at all — no $\text{Rep}(\Psi)$, no method type, no evaluator, no $\ulcorner \Pi \urcorner$. It is statable in a strictly smaller language than Theorem 8. **Re-checked after Theorem 13 (2026-09-09).** This claim was originally witnessed by the transcription that Theorem 13 refutes, so it was re-run against the demoted shape: `Theorem7_shape`, carrying all five restored ingredients as parameters, elaborates in a file where `Layer` is not defined at all, with no reference to $\text{Rep}$, methods, or the encoding anywhere in it. The signature claim therefore survives its input's refutation, and survives it in stronger form — the earlier witness lacked the ingredients, this one carries them. Limit of the check: the ingredients are present as *parameters*, so it establishes that the shape is layer-free, not that a faithful instantiation must be; $P$, the basin and $\rho_\infty$ are all substrate-side, so there is no evident route for the representation apparatus to enter, but Problem 29 owes the instantiation that would settle it. So below the carrier boundary self-representation is not merely unused but *unmentionable*: the vocabulary in which Theorem 8's construction is posed does not exist in the signature. That is why the two columns cannot bleed into one another, and why no ingredient of the ergodic side could re-enter the constructive one — the question was tested and answered negatively (Problem 25, discriminator).

Theorem 7 is a **reachability** result: it proves an ergodic walker will find $\Psi^*$ — with probability bounded below by the basin's stationary mass, approaching certainty only as $\Delta P/D \to \infty$ (Theorem 14) — but $\Psi^*$ must already be there. It cannot account for a system that *builds its own next constraint* — and every layer from the gene upward does exactly that. Theorem 8 supplies the missing construction. Conversely Theorem 8 alone would license unbounded, undirected extension; it produces a candidate, not a survivor.
...
**Statement**: In the framework as formalized, an affordable method can decide anything a function can compute. Consequently no claim of the form *no affordable method decides $X$* is provable, for any $X$; every such claim is a stipulation about the method set, not a result. *Relabeled 2026-09-10 (target (xiv)):* this is a theorem about cost left untied to computation. Under a cost tied to what methods compute through a counting law, the existence of an inaccessible predicate is provable (Problem 35); the refutations stand, at their own cost, as the statement that affordability constrains nothing about cheap predicates.
...
### Theorem 11: Self-Application

**Statement**: For every layer $L$ and every method $m$ of $L$ — affordable or not — $m$ does not compute the predicate

$$D_L(y) \ :=\ \forall m',\ \text{decode}(y) = \text{some}\ m' \rightarrow \text{run}(m', y) = \text{false}$$

("the method encoded by $y$ rejects its own code").

**Proof**: instantiate at $y = \text{encode}(m)$ and apply the round-trip law. The biconditional becomes $\text{run}(m, \text{encode}(m)) = \text{true} \leftrightarrow \text{run}(m, \text{encode}(m)) = \text{false}$, which no Boolean satisfies. Machine-checked and **axiom-free** (`formal/BoundaryResidue.lean` §15). $\blacksquare$

**Why this is the diagonal transposition and the earlier one was not**: Theorem 8 (i)'s earlier transcription asserted that some point's *identity* predicate is method-inaccessible. That is false under every cost model — comparing an input to a constant is cheap in every complexity theory ever proposed — and Corollary 10.3 refutes it even with cost axioms assumed. The Gödel construction was never about identity; it is about **self-application**, and the moment the predicate is stated correctly the theorem needs no cost theory at all.

**Two consequences worth separating.** First, the diagonal mechanism is **cost-free in the literal sense**: affordability never appears in the statement or the proof, so the theorem holds for every method a layer has, at any price. Second, this is the framework's first *positive* formal result about itself — everything prior was a representation choice, a refutation, or a stub — and it was available in $\text{decode\_encode}$ from the first commit, waiting for the predicate to be stated correctly.

### Theorem 12: No Omniscient Layer

**Statement**: no layer computes every Boolean function on its own representation space: there is no layer $L$ such that for all $f : \text{Rep} \to \text{Bool}$ some method computes $f$.

**Proof**: the computable diagonal $y \mapsto \lnot\,\text{run}(\text{decode}(y), y)$, evaluated at its own computing method's code. Machine-checked (`[propext]`). $\blacksquare$

**Reading**: this is the cardinality bound implicit in $\text{decode\_encode}$ — the round-trip law forces $\text{encode}$ injective, hence $\text{Method} \hookrightarrow \text{Rep}$, and Cantor does the rest. It is the **only** limit on method-omniscience the framework currently has, and it is bought by self-encoding rather than by any cost notion. Theorem 10 said the frame cannot express hardness; Theorem 12 says self-encoding nonetheless purchases exactly one impossibility, and Theorem 11 says that impossibility is the diagonal one.

**Corollary 10.3 (the cost-hypothesised diagonal restatement is false)**: exhibited by a parity-priced layer where costs are unbounded — so the cost axioms hold — while every identity predicate has a free decider. Machine-checked, with the cost axioms *proved* for the witness and the affordable decider exhibited, so the refutation is not vacuous.

**Corollary 10.4 (the cost-hypothesised opacity restatement is false)**: same pricing, with an element no method can ever fire on, so the hardness hypothesis is satisfied rather than dodged. Machine-checked, cost axioms proved.

**Proposition (relativized Blum-2 excludes point-hardness)**: under the arithmetized second Blum axiom — *the layer can read a method's price off its code* — no element that **is** a code can be incompressible, since the price query at a method's own cost fires on its own code. Incompressibility survives only at non-codes. Machine-checked, axiom-free.

**Consequence, and it is the batch's sharpest**: on any layer whose $\text{decode}$ is total, adding Blum-2 makes the opacity transposition's hardness hypothesis *unsatisfiable*, so the statement holds vacuously. That is worse than being refuted. It shows that point-incompressibility is **not a formalization of Razborov–Rudich hardness at all** — RR hardness is indistinguishability of *distributions*, a statement about measure, not the invisibility of a point.

---

### Theorem 13: Theorem 7's Transcription Was False

**Statement**: the formal transcription of Theorem 7 that stood in the skeleton is refutable. There is a substrate and an occupation function satisfying the transcribed ergodicity hypothesis exactly, for which the conclusion fails.

**Proof**: take total mass $1$ on a scale where the transcription's $\text{one}$ is $10$. Occupation converges to the stationary measure as demanded, and $\text{one} - \varepsilon$ is still never exceeded, because nothing in the transcription related the measure's mass to $\text{one}$. Machine-checked (`formal/BoundaryResidue.lean` §16). $\blacksquare$

**The dropped ingredient is normalization** — $\rho_\infty$ is a stationary *probability* measure in Theorem 7, and the transcription let mass and scale float free. Four further ingredients had been lost in transport, any one of which would have made the transcription unfaithful even had it been true: the universal over initial conditions (*that any start reaches the basin is the theorem's content*); the parameter $\theta$ and its threshold $\theta > \theta_c$, so the conditionality vanished; the link between the basin and a non-degenerate maximum of $P$ with depth $\Delta P > 0$, leaving `basin` a free predicate and $P$ unused; and ergodicity itself, which the transcription replaced with Birkhoff's *conclusion* — an intermediate step of the manuscript's own proof, assumed as a premise.

**Boundary, as with the others**: Theorem 7 is untouched. Morse theory, Birkhoff and Kramers are not in question, and neither is the manuscript's proof sketch. What failed is the transcription, for the third time in three attempts. **Re-established on a genuine measure (2026-09-10).** The refuting witness first carried a "measure" that gave the empty set mass one (Problem 32). Repaired to a genuine point mass — still one unit on a scale of ten, so still unnormalized — the refutation goes through unchanged in substance. Only the witness's measure and the proof's dependence on classical choice changed.

**Consequence for the skeleton**: with this demoted to a committed shape (`Theorem7_shape`, every dropped ingredient an explicit parameter), the file now carries **no `sorry` at all**. It asserts nothing it has not proved. That is a smaller achievement than it sounds — it was reached by removing false statements, not by proving true ones — but it does mean the register boundary is currently clean.

---

### Theorem 14: Theorem 7 As Stated Is False

**Statement**: the conclusion of Theorem 7 — *for any $\epsilon > 0$ there is $T_\epsilon$ with $\Pr[\Psi_t \in \text{basin}] > 1 - \epsilon$ for all $t > T_\epsilon$* — does not follow from its hypotheses, and is false for processes satisfying them.

**Proof**: by a two-state chain with strictly positive transition probabilities in both directions — irreducible, aperiodic, unique strictly positive stationary distribution, hence ergodic under every reading of the word. Stationary mass on the basin is $\tfrac12$. Every hypothesis is exhibited rather than assumed, including normalization and a non-empty basin containing $\Psi^*$. For $\epsilon < \tfrac12$ no $T$ exists. Machine-checked (`formal/BoundaryResidue.lean` §18). $\blacksquare$

**This is a different kind of finding from Theorem 13, and the direction is reversed.** Theorem 13 said a transcription had lost conjuncts the manuscript supplied. Here the transcription was checked item by item against an enumeration written in advance by someone who had never transcribed it (§17, 36 items), and it is *faithful*. What fails is the theorem. **The defect is in the informal register, and this appendix should say so as plainly as it said the reverse ten times.**

**Where the over-claim enters**: $\Delta P$ and $D$ appear in the hypotheses and *nowhere in the conclusion*, so the conclusion is quantitatively independent of the noise level. But the proof sketch's own reachability step computes a stationary mass $\rho_\infty(\text{basin}) = Z^{-1}\int_{\text{basin}} e^{P/D}$, which is strictly less than $1$ whenever the noise is nondegenerate, since $e^{P/D} > 0$ everywhere. A process cannot spend more than $\rho_\infty(\text{basin})$ of its time in the basin, so no $\epsilon$ below $1 - \rho_\infty(\text{basin})$ is achievable. The sketch supports *fraction $\to 1$ as $\Delta P / D \to \infty$*; the statement claims *fraction $> 1 - \epsilon$ for every $\epsilon$, at fixed $D$*. Those are different theorems and the sketch proves the weaker one.

**A second gap, independent of the first**: the conclusion is pointwise in $t$ — a statement about marginal laws, which requires mixing. Birkhoff delivers time averages. Even with the $\epsilon$-bound corrected, the hypothesis as written does not reach the conclusion's form.

**Scope**: Morse theory, Birkhoff and Kramers are not in question, and neither is the phenomenon Theorem 7 describes. What is refuted is the *quantifier*, not the idea that deep basins in low noise are occupied nearly all the time. The corrected statement is given at Theorem 7 itself.
...
25. **Is self-opacity derivable from RE primitives?** — Conjecture 7 (b) imports its second mechanism from complexity theory: a constructive, broad method class cannot settle a boundary object because it would compress the $\Pi$-incompressible. Derive this from RE's own definitions of $R$, $\Pi$, and $S$ — or show that opacity is a fact about specific layers that RE can host but not generate. A positive result would give RE a second growth-relevant limitation theorem alongside Theorem 8; a negative one bounds how much of complexity theory the framework can claim to explain rather than merely accommodate.
    ...
    **Prerequisite.** Before the derivation can be attempted, "$\Pi$-incompressible" must be a well-defined predicate over $\text{Rep}(\Psi)$ — a gap adjacent to Problem 22's undefined-vs-zero construction and not covered by it. Time-bounded Kolmogorov complexity supplies the candidate formalization, and the Minimum Circuit Size Problem supplies the sharpest instance: given a truth table, decide whether a small circuit computes it. MCSP *is* a layer inspecting its own compressibility, and Kabanets–Cai (2000) made the connection to the opacity mechanism explicit — a natural property is essentially an efficient MCSP-style algorithm. The representation choices this forces are recorded in `formal/BoundaryResidue.lean`, which proves nothing and is annotated throughout with which open problem each choice decides. **A candidate measure, 2026-09-10:** the stationary measure pushed forward through A2's projection is additive once the projection is a function, and gives largeness a non-vacuous referent at the positive-mass threshold (skeleton §§25–27; Problem 31 records the restriction it needed).
...
26. **The two $P$'s** — Theorem 7 writes $P(\Psi; \theta)$ for a potential on the substrate state-space $\mathcal{X}$; Definition 1 writes $P(\Phi) = R \cdot \Delta H \cdot S$ for the emergence potential of a candidate coherence. These are different functions on different domains, and the framework denotes both by $P$. The transcription in `formal/BoundaryResidue.lean` §6 could not close the gap and declined to invent a law: no equation relates `Substrate.P : State → Val` to `Lattice.P : {x // Defined x} → Val`, and the bridge structure carries no coherence condition. State the induction — how a candidate coherence's emergence potential is determined by the substrate states that formulate it — or establish that the two quantities are independent and the shared notation is an error. This is Problem-21-shaped: a bridging law currently asserted by notation rather than exhibited, and it is load-bearing wherever an argument passes between Theorem 7's regime and Definition 1's.
    ...
    **Status, 2026-09-10 (target (xii)).** A dependency enumeration over the formal record, done at the level of its terms rather than its text, splits this problem three ways. *The partiality-below clause* is blocked: every existing result that bears on it fails for the reason Theorem 10 gives — an affordable method can compute anything — and would fail the same way for any bridging law, so it waits on a cost notion that constrains computation — what cost prices is now Problem 35. *The bridging law proper* has no existing result on either side: no theorem in the record has ever been about Definition 1's potential at all. It is being probed with the stationary-measure-weighted average as the candidate. *The vocabulary clause* has no formal referent and is Problem 33. **(xii-a) outcome, 2026-09-10: a genuine condition, consistent and discriminating.** With every layer-side quantity held fixed — the lattice, $R$, $\Delta H$, $S$ — the bridging condition accepts one substrate and rejects another, both conforming to A2 and both carrying a genuine measure, at a point of positive fiber mass. On that evidence it says: *Definition 1's potential at a defined structure equals the stationary-weighted average of the substrate potential over the states that formulate it.* That is the most it can say while $R$ and $\Delta H$ have no referents (Problem 34). Comparing the two potentials also turned out to require identifying the lattice's values with the substrate's, which the formal record keeps as separate types — one more thing the shared letter had concealed. The satisfying substrate is the weakest one possible — a constant potential equal to the layer's value — so the evidence is in the *pair*, not in either member. **What the shared letter was doing.** Six hypotheses, in the order the formal record uncovered them: that the two potentials have different domains, substrate states against candidate coherences (this problem as first posed); that the projection between them is single-valued, total and surjective — three properties of a function that the record had carried as a bare relation (Problem 31); that the stationary measure is a measure at all (Problem 32); and that the two potentials take values in one type. Shared notation was doing the work of six hypotheses, none of them stated.
...
28. **Opacity is not yet hostable — demote, with the shape committed** — the transposition of Conjecture 7 (b) into the formal frame failed twice, and neither failure is repairable by adding hypotheses. Its logical shape was inverted: the frame stated *large $\Rightarrow$ not constructive*, where Razborov–Rudich concludes *constructive $\wedge$ large $\Rightarrow$ not useful*; the dropped **usefulness** conjunct is exactly what made the frame's version false, since the trivially broad predicate is cheaply computable and useless. And its hardness notion was wrong: point-incompressibility is not RR hardness, as the Blum-2 proposition above shows by making it unsatisfiable. Restating with usefulness bolted on would repeat the diagonal arm's error one conjunct over — fixing a statement whose remaining ingredients have no referents — so the conjecture is **demoted rather than restated**, with its shape committed as an asserted-of-nothing definition (`RR_shape` in `formal/BoundaryResidue.lean`) so the inversion cannot recur silently and so Problem 25 has a concrete target to instantiate.

    **The dependency collapses to one object.** Largeness needs a measure on $\text{Rep}$ — already Problem 25's prerequisite. RR hardness is indistinguishability of distributions, also a measure statement. Usefulness is defined relative to hardness. All three placeholders therefore wait on the same thing: *opacity becomes hostable when Problem 25's measure exists, and not before.* Hostability is compile-decidable — the instantiated shape either elaborates or does not — which is the success criterion. **Update, 2026-09-10:** one slot of three now has a referent. Problem 25's pushforward probe gives largeness a candidate from inside the framework — the stationary measure pushed through A2's projection, at the positive-mass threshold — and the instantiated shape elaborates with that slot filled. Hardness and usefulness still have none. The dependency on Problem 25's measure is therefore partly discharged, not fully: what remains is a hardness notion about distributions and a usefulness notion relative to it.

    **Boundary.** Razborov–Rudich is untouched, and Conjecture 7 (b) rests on it exactly as before. What failed is this frame's transcription, twice. Reading the demotion as a retraction of the opacity mechanism mistakes a demotion of a transcription for a demotion of the mathematics
29. **A faithful transcription of Theorem 7 is owed** — the first attempt was false and is demoted to `Theorem7_shape`, which carries every dropped ingredient as an explicit parameter: normalization, the initial-condition universal, $\theta$ with $\theta > \theta_c$, the basin as the basin of a non-degenerate maximum of $P$ with $\Delta P > 0$, and ergodicity of the Langevin process rather than Birkhoff's conclusion. Instantiate the shape so that each parameter has a referent, and check the result against the manuscript statement conjunct by conjunct rather than by reading. The success criterion is the one Problem 28 uses: the instantiated shape elaborates, and no cheap witness refutes it. **Note the base rate.** Three transcriptions have been attempted in this file and three were false — two by dropped conjuncts, one by a wrong predicate — and all three survived multiple readings by both author and reviewer before a machine was aimed at them. Transcription in this framework should be treated as an error-prone operation requiring a refutation attempt, not as a clerical step.

    **A red flag with a name.** The fifth defect — ergodicity replaced by Birkhoff's conclusion — is a proof's intermediate step promoted to a premise, which is the same shape as a hypothesis stub strong enough to imply its own conclusion. That it turned up in a *transcription* rather than in a stub shows the mode is not confined to the place we had learned to watch for it. So: a faithful transcription's premises must be the *manuscript's* premises, and any premise that appears as a lemma inside the manuscript's own proof is a red flag by construction.

    **The countermeasure, since the failure mode is uniform.** All ten ledger entries are losses in transport — a dropped conjunct, quantifier, constraint, normalization or link. Nothing was ever fabricated; the boundary leaks by *omission*, in one direction. The mechanical answer: before writing any term, enumerate from the manuscript statement every quantifier with its order, every hypothesis, every constraint on a defined object, and every cross-reference to a named quantity — as a list, committed first — then check each item off against the finished term. Finite list, each item present-or-absent in the code, which is as compile-decidable as fidelity gets without a formal manuscript. Then the refutation attempt. Three for three suggests such a list would have caught all fifteen defects across the three attempts, because every one of them existed in the prose and not in the term
30. **$\theta_c$ is undefined, and $\theta > \theta_c$ is ill-typed** — surfaced by the Problem 29 enumeration (§17, items 5 and 8) before any code was written, and independent of Theorem 14. The threshold $\theta_c$ appears in Theorem 7's hypothesis (i), in its implication, in its contingency note and in Theorem 8's comparison table, and is **defined nowhere**. Worse, the comparison itself has no referent: A6 gives $\Omega = (\mathcal{M}, \mu, \mathcal{F})$ — a manifold, a measure, a filtration — and none of these is an order, so $\theta > \theta_c$ cannot be read at all without either designating $\theta$ a scalar coordinate or equipping $\Omega$ with an order. Supply both the threshold and the relation, or restate hypothesis (i) in terms that A6 supports. Note the standing: hypothesis (i) of RE's *strongest necessity result* has been ill-typed since it was written, and the transcription had to stub the comparison as an opaque predicate to proceed. Two further undefined terms were found in the same pass and are lesser: `basin`, characterized only inside the proof sketch, and "carrier-class type", which may have a referent elsewhere in the manuscript but has none in Theorem 7's vicinity
31. **The projection is a relation in the formal record, and A2 says it is a function** — A2 states that $\Pi$ is a *surjective function* from memory states to coherent structures, many-to-one. The joined signature that the formal record uses to connect substrate and layer carries only a bare relation, `formulable`, with no hypothesis making it single-valued, total, or surjective, and its only witness relates every state to every structure. This matters as soon as anything is transported along $\Pi$: pushing the stationary measure forward onto representations is **additive** only if the relation is single-valued, and **normalized** only if it is total; surjectivity is needed for neither. Found by the first check of Problem 25's pushforward probe and logged before the pushforward was defined. The probe proceeds under `SingleValued` and `Total` as explicit hypotheses citing A2. Open question: whether the relation should be restricted to a function everywhere in the joined signature, or whether some part of the framework genuinely needs a state to formulate more than one structure — in which case A2's "function" is the thing to revise
32. **The stationary measure is not a measure in the formal record** — the structure that carries Theorem 7's $\rho_\infty$ requires only that the basin have positive mass. Nothing gives the empty set mass zero, nothing makes the mass of a disjoint union the sum of the masses, and one witness in the record assigns mass one to every set. "$\rho_\infty$ is a measure" was a gloss. The repair has two steps, both representation choices, and the first blocks the second: the value type has no addition, so additivity cannot even be *stated* on the general structure. It needs an addition — with only such laws as some proof demonstrably uses, each named by the theorem that needs it — and then fields for the empty set and for additivity, at which point the defective witness stops typechecking, correctly. **This blocks Problem 26**, whose aggregation consumes $\rho_\infty$: an aggregation against something that is not a measure is not the object the manuscript means. Distinct from Problem 29's normalization defect, and found differently — by accident, while auditing a witness for another property. **Resolved, 2026-09-10 (target (xi), outcome R−).** The structure now carries an addition — with no laws, since no proof needs one — and requires the empty set to have mass zero and masses to add over disjoint sets. The two witnesses whose "measure" was not one were repaired with genuine point masses, and the old constant measure is kept on record as ruled out by both new conditions. Every result re-established, the two numbered ones among them directly on their own repaired witnesses. The pushforward's additivity is now derived from the structure itself rather than assumed, and remains free of classical choice. **Problem 26 is unblocked.**
33. **"Restriction to the layer's vocabulary" has no formal referent** — Problem 26's constraint asks that the induced potential, restricted to the layer's own vocabulary, stay partial. The formal record has no operation of restriction to a vocabulary, and a probe of it could neither pass nor fail. Candidates the manuscript could supply — none is chosen here: *closure under the layer's own signature*, meaning what can be built from the layer's own components alone; *method-availability*, meaning what an affordable method of the layer can compute, which is the sharpening the record already tried and which is blocked on Theorem 10's cost debt; and *definability* in some fixed logic over the layer's structures. No probe until the manuscript says which it means **Consolidated, 2026-09-10:** its method-availability candidate turns on what cost prices, which is Problem 35.
34. **$R$ and $\Delta H$ have no referents in the formal record** — Definition 1's potential is the product of reusability (Definition 2), entropy reduction (Definition 4) and structural compatibility (Definition 3). The formal record carries only the last as a component of the lattice; the first two enter as free parameters. So every statement about Definition 1's potential there is a statement about an unconstrained product, and the bridging law of Problem 26 can be shown consistent and discriminating but can say nothing about reusability or entropy reduction themselves. Give each a referent — which requires deciding, for reusability, what utility and cost are measured against, and for entropy reduction, whose entropy and conditional on what — and the bridging law acquires content about them. Recorded as the ceiling pre-registered for Problem 26's probe, not discovered after it **Consolidated, 2026-09-10:** reusability's denominator is Definition 2's $C$, and whether that is the layer's cost is Problem 35's question. A second bill follows whichever answer comes: $\Delta H$ is a conditional Shannon entropy, which needs logarithms over the reals, and the formal record is free of Mathlib with its values kept abstract. Giving $\Delta H$ a referent will therefore bring either Mathlib into the record or entropy in as an axiomatized stand-in — the trade the note on the price of honest measures records four times.
35. **What does cost price: a structure or a computation?** — The formal record's layer puts a price on each method, $\text{cost} : \text{Method} \to \mathbb{N}$, tied to nothing the method computes; Theorem 10 is the consequence. The manuscript has its own candidate for such a price — Definition 2's $C$, the cost of creating and maintaining a structure — but has never said whether the layer's cost is that, and nothing in the record identifies them. Three blockers turn on the answer: Problem 26's partiality-below clause, which waits on a cost that constrains computation; Problem 33's method-availability candidate; and the denominator of Definition 2's reusability (Problem 34). Problems 33 and 34 stay open under their own numbers and point here. *Consolidated 2026-09-10 from three separately filed blockers, on the reviewer's ruling. A kernel-level dependency closure finds the cost field under 27 statements of the record, three of them among the six frame definitions (undecidability, incompressibility, and the Razborov–Rudich shape), and under none of its open conjectures.* The readings the answer would select between:

    **(a) An untied price** — Definition 2's $C$, and what the record has now. Nothing is re-typed, and reusability's denominator acquires a referent. The price: Theorem 10 then stands permanently, so no claim that an affordable method *cannot* decide something is provable, and the partiality-below clause stays blocked for good.

    **(b) A per-input measure tied to computation** — Blum's axioms (1967), in which a method's cost is its resource use on each input, defined exactly where the computation halts. The record's methods run on every input and are priced once each, so Blum's first axiom has nothing to say about them. Two transcriptions are possible, and choosing between them belongs to this answer rather than to the file. *Replace* the price with the measure: this re-types the layer, and with it three of the six frame definitions and the 27 statements, and moves the audit baseline. Or *add* the measure beside the price with the frame frozen: affordability then still reads the old price, so unless the manuscript also says how the two are linked, Theorem 10 applies verbatim and the new measure prices nothing.

    **(c) A per-method price tied to computation** — the notion Razborov–Rudich actually use, circuit size: priced once per method, like the record's cost, but constrained by what methods of a given price can compute. Nothing is re-typed; the link is a new axiom on the pair (run, cost). Recorded as a candidate, not checked: whether any such axiom escapes Theorem 10's witness is unknown. *The trade, stated for the author and not as a recommendation:* (c) is the reading that matches the source of the opacity mechanism, and the only one under which cost acquires computational content without re-typing the frame or a second decision — the beside-transcription of (b) avoids re-typing too, but only with a link the manuscript must also supply. And (c) is unchecked: if it is chosen, the first target is its check against Theorem 10's witness, and that check can fail.

    **The central fact: the unblocks are not monotone.** (a) gives reusability's denominator a referent and forecloses the other two. (b) or (c) makes the partiality-below clause and Problem 33's method-availability candidate probeable, and leaves reusability's denominator without a referent unless the manuscript also says that Definition 2's $C$ is the tied cost. Only an answer that says so — (c), with $C$ as the price — unblocks all three at once.

    **Decided in the loop, 2026-09-10 — reading (c); not yet manuscript text.** Adopted on the reviewer's ruling under the author's direction, and confirmed by the author: cost is a per-method price tied to what methods compute, and Definition 2's $C$ is identified with the cost of the method that carries a structure. The reasons: (a) would make Theorem 10 permanent, so self-opacity (Conjecture 7 (b)) could never be hosted; (b) needs per-input semantics that the record's total `run` cannot carry without re-typing the frame, and it is not the cost Razborov–Rudich use; (c) needs no re-typing and is theirs. The manuscript owes the sentence that makes this identification; until it is written, the formal record carries it as a decision of the loop, reversible by the manuscript. It is also unchecked: target (xiv) tests it against Theorem 10's witnesses, and if the test fails the question reopens with the failure recorded. *Checked the same day (target (xiv)): it holds.* With the tie and the counting law, the existence of a predicate that no affordable method decides is machine-checked in general, free of classical choice — Shannon's 1949 counting argument in the frame's terms. Every transparent layer that refuted the unhypothesized conjectures admits a tie at its own cost, under which all its predicates are cheap, so those refutations stand. And one layer, tied two ways at the same cost, flips from no inaccessible predicate to one as the price function changes: on a single layer the stipulation moves into the price function, and the content lies in the theorem's generality. This establishes that tied cost has content. It contributes nothing toward separating P from NP: the record has no Boolean functions on $n$ bits, no circuits, no time bounds and no pseudorandom generators. Its design constraint is recorded there: a price tied to computation by a free function constrains nothing, since every layer satisfies the tie with every price zero, so the price function carries one law — a counting law, Shannon's, under which most predicates are expensive.

**Remark (open, and possibly nothing).** The two remaining blockers may be one object, and the frame may already contain it. Problem 25 needs a measure on $\text{Rep}$; the discriminator established that $\rho_\infty$ lives on $\text{State}$ and refuted the reading that it fills the largeness slot directly. But A2's projection maps substrate states to formulable structures, and a measure on $\text{State}$ together with that map yields a measure on $\text{Rep}$ by **pushforward**. That is a third role, distinct from both predecessors: not the identity of carriers the grave marker refutes, and not the bridging aggregation Problem 26 owes. It would give largeness content — a property is broad when its $\Pi$-preimage carries $\rho_\infty$-mass — and give RR hardness a distribution to be indistinguishable *from*. If it goes through, opacity becomes hostable in the *joined* signature, one level above where it was originally stated, which is where escape-by-extension says boundary questions become well-formed.

    *Explicitly distinguished from the resolved remark above*: that reading placed $\rho_\infty$ on $\text{Rep}$; this places $\Pi_*\rho_\infty$ there, and the difference is a map. *Pre-registration, for whenever it is probed*: (i) does the joined signature supply $\Pi$ as a total map or only fibers — if only fibers, the pushforward needs a choice, and that choice is a representation decision to log; (ii) is the pushforward largeness non-vacuous, guarded by exhibiting both a positive-mass and a zero-mass property; (iii) does hostability then elaborate, with `RR_shape` instantiated by pushforward largeness — the compile-decidable criterion Problem 28 already names. *Prior, distrusted*: (i) will be "fibers only", making this a (C) with a logged choice before it is a result. Possibly nothing — but it is the first candidate for Problem 25's measure that comes from inside the file rather than from importing a theory, and the last positive result came from a law that had been in the file since the first commit.

    **Resolved, 2026-09-10 — toward the reading it hoped for, with one repair on the way** (pre-registered and run in `formal/BoundaryResidue.lean` §§25–27). Recorded as outcome (A), under a hypothesis the manuscript already states and the transcription had dropped. Item (i) landed on the "fibers only" side the prior predicted, but for the wrong reason — it predicted a choice to log and got a loss to repair: the formal record carried $\Pi$ as a bare relation although A2 says it is a function, a transcription loss now logged as Open Problem 31. Restricted to a function as A2 requires, the pushforward $\Pi_*\rho_\infty$ is a genuine additive measure, normalized by totality; it gives largeness a non-vacuous referent, with a property of positive mass and a non-empty, formulated property of zero mass exhibited on the same witness; and the Razborov–Rudich shape instantiated with it elaborates. On the transparent layer that refuted every earlier transposition it is *consistent*, and constrains usefulness on one property when hardness holds — nothing stronger, since satisfiability is consistency and not content. What the probe did **not** do is supply hardness or usefulness — two of the shape's three slots remain without referents — so opacity is now partly hostable, not hostable. Largeness here is positive mass, which is weaker than Razborov–Rudich's fraction-largeness.

**Remark (open, and possibly nothing).** Formalizing this framework honestly has now forced two rediscoveries, in historical order. Lean's classical foundations forced the frame to reify its methods — you cannot *state* undecidability without a machine model, which is the 1936 lesson. Theorem 10 then forced the frame to give cost operational meaning — you cannot *prove* inaccessibility without a cost measure tied to computation, which is the Hartmanis–Stearns and Blum lesson. **Corrected 2026-09-09, not deleted, per the retention convention above.** The second rediscovery was mis-identified. It was not Hartmanis–Stearns: the cost probe showed neither conjecture's debt was cost-shaped. The diagonal owed *self-application*, which had been sitting in the round-trip law since the first commit (Theorem 11), and Blum entered as a mis-named debt and leaves as an honest stub with exactly one real finding attached — that it excludes point-hardness, which is why RR hardness must be set-level. So both rediscoveries are the same one: the diagonal lemma, met first as a constraint on *stating* undecidability and then as the thing that was actually owed. Whether that is evidence the boundary-residue material tracks something real, or the discovery that the frame is reconstructing standard computability theory under new labels, is not decidable from what is now one instance rather than two, and the flattering reading is the one to distrust. Filed at the third tier next to its sibling. The *direction*, however, is unambiguous either way: the restated conjectures should hypothesize Blum-style axioms on the pair (run, cost) rather than a concrete machine model, keeping the frame abstract while paying the debt Theorem 10 called in. **Qualified 2026-09-10, not withdrawn.** That direction presupposed that cost is to be tied to computation, and per input. Both are open: whether cost prices a structure or a computation is Problem 35, where Blum's axioms are one of three readings.
...
**Handoff, 2026-09-10 — the verification loop pauses here.** *What the record can now do.* It can host: this cycle it gave referents to three slots the manuscript's statements use — largeness, as positive mass under the pushforward of the stationary measure (Problem 25); the bridging law between the substrate's potential and Definition 1's, as a condition shown consistent and discriminating (Problem 26); and the stationary measure, as a measure (Problem 32). What it cannot yet do is derive. Every derivation still open on the boundary-residue side routes through the unanswered question of what cost prices — the partiality-below clause through a cost tied to computation, opacity through its constructivity clause, which reads affordability, and reusability through Definition 2's $C$ — though cost is not the only thing some of them lack. One exception, so that sentence is not read as universal: the general finite forms of Theorem 7's Parts A and B depend on no cost at all, and are open for want of a proof — a finite Doeblin or Perron–Frobenius argument not yet formalized — not for want of a decision. They are the only open statements in the formal record, and the only work the loop could take up without the manuscript, should its author prefer that to waiting; that choice is the author's. *Taken up the same day, on the author's go (target (xiii)):* both are proved on the record's class, so the formal record now carries no open statement, and Problem 35 is the only thing the loop waits on. *Answered in the loop the same day — reading (c); see Problem 35.* *The one question, answerable in a sentence:* does cost price a structure or a computation — a price per method, which is Definition 2's $C$ and what the record has; a measure per input, which is Blum's; or a price per method tied to what methods compute, which is circuit size and the notion Razborov–Rudich use? Or a fourth thing, in which case say what. *What each answer costs.* (a) Nothing is re-typed, and Theorem 10 becomes permanent. (b) A re-typed frame — three of its six definitions and 27 statements — or else a link to affordability that the manuscript must also supply. (c) One new axiom, and a first target that can fail: (c) matches the source of the opacity mechanism and is the only reading that gives cost computational content without re-typing the frame or a second decision, but it has not been checked against Theorem 10's witness. A second bill follows any answer: $\Delta H$ needs Mathlib or an axiomatized entropy (Problem 34). *What each answer unblocks.* (a): reusability's denominator. (b) or (c): the partiality-below clause and Problem 33's method-availability candidate. All three only if the manuscript identifies $C$ with a tied cost. *The loop resumes on the answer, and not before:* nothing in the formal record will be probed toward cost until the manuscript's author has said which reading is meant (Problem 35).

*On weighing the priors behind this record.* From the legality audit of target (x) to here, the reviewer's premises about what the record contained were wrong four times: twice about which sections held which results, where the record's dependency graph said otherwise; once taking the cost stub for a transcription item; and once taking every open derivation to route through cost. A fifth came in the guide for target (xiv), which named the transparent identity layer as the witness behind Theorem 10's own results; that layer carries only Corollary 10.1. Its outcome priors have been scored wrong four times in the formal record, the fourth at target (xiii): it held that the proof needed exactness at every set, reasoning about division before additivity, and the compiler showed that exactness at single states suffices — order of operations again, this time in the mathematics. A fifth came at target (xiv): it held the counting result non-constructive by necessity, and the audit found it free of choice. Mine were wrong on a premise once in the same stretch — taking Blum's per-input measure for the only cost tied to computation — and my process slips are counted in the formal-record notes under Theorem 7, in the entries on surface text and order of operations. Pre-registrations are therefore drafted from the record's dependency graph and not from recall, and a prior carries the weight its tally gives it.
...
The current formal system has 3 primitives, 7 axioms, 17 definitions, 14 theorems, 7 conjectures, 9 predictions, and 35 open problems. While internally consistent, this is a large surface area. A Sheffer-stroke reduction asks: can the entire formal system be derived from a single rule and a single seed — analogous to how Sheffer's NAND generates all of propositional logic, or how Odrzywołek's $\text{eml}(x, y) = e^x - \ln y$ generates the elementary functions?

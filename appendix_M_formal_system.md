...
The current formal system has 3 primitives, 7 axioms, 17 definitions, 14 theorems, 7 conjectures, 9 predictions, and 32 open problems. While internally consistent, this is a large surface area. A Sheffer-stroke reduction asks: can the entire formal system be derived from a single rule and a single seed — analogous to how Sheffer's NAND generates all of propositional logic, or how Odrzywołek's $\text{eml}(x, y) = e^x - \ln y$ generates the elementary functions?

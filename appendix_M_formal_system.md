...
- **What is and is not machine-checked.** Parts A and B are stated generally in the record on finite spaces, as conjectures with proved instances beside them. **Part C has no Lean marker**: its general statement needs $\mathbb{R}^n$, which the Mathlib-free skeleton cannot host, and the record holds only the refutation of its predecessor and its finite confirmations. The "not machine-checked" sentence at Part C is its marker.

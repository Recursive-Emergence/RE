...
So, exactly: **class** E^{NP‖} (parallel/non-adaptive NP queries); **circuit model** SV-nondeterministic (single-valued nondeterministic); **size** exponential; **queries** non-adaptive; **error** one-sided relative. *(Worst-case vs a.e./i.o. quantifier: Cor 3.3 says "worst-case hard"; I did not verify an a.e. form, so it is not claimed.)*
...
- **i.o. vs a.e.:** AABHvM Theorem 3 concludes **a.e.** ("all but finitely many input lengths"); SU05's worst-case/a.e. quantifier **not verified**, so not claimed.

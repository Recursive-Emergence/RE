...
- **CORRECTION, caught by the reviewer after this section was first committed.** The Definition 2.4 quote above originally read "size at most 2^{n^ε}". **That was wrong, and worse, it was a superscript I reconstructed inside quotation marks** after the text extraction dropped it. The exponent is **linear: 2^{εn}**, established three ways — (i) raw-mode extraction gives the glyph order `2`,`ε`,`n`; (ii) the PDF's superscript span for Def 2.4 is 12px wide while rendering only "n", which is 7px in that same font, so the missing glyph sits *inside* the exponent; (iii) SU05's introduction states the same hypothesis independently as "the language cannot be computed by size 2^{εn} circuits, for some ε > 0". *(The constant's glyph is unmapped by every extractor tried here and is rendered ε by convention; nothing in this report depends on the letter, only on the exponent being linear.)* **This matters:** 2^{εn} is linear-exponential, the same scale as Korten Thm 10's 2^{Ω(n)}, whereas 2^{n^ε} would have been sub-exponential and would not have composed with the door. **Practice line: never reconstruct a dropped superscript inside a quotation; mark it lost or check a second extractor.**

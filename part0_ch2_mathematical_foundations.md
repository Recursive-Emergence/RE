...
The strength ordering in that table is not an observed contrast but a consequence, and it can be read off the premises. The diagonal mechanism needs one thing: an encoding under which the layer can quote its own evaluator. The opacity mechanism needs that, *plus* a largeness notion over the space of candidates, *plus* a hardness hypothesis. Conditionality is the extra hypothesis burden — three premises against one — so the row reporting that one mechanism is unconditional and the other conditional is explained by counting, not merely recorded.

Formalization has since sharpened both sides of that count, and widened the gap. On the diagonal side the single premise is now *proved sufficient*: self-application is unprovable by any method a layer has, at any price, from the encoding alone (Appendix M, Theorem 11 — and note that affordability never enters, so the diagonal mechanism is cost-free in the literal sense). On the opacity side the count rose rather than fell: largeness, hardness, and usefulness all turn out to be measure-relative, and none of the three yet has a referent, so the mechanism is not currently *statable* in the formal frame at all (Open Problem 28). The asymmetry the table records is therefore not a difference in confidence about two comparable claims. One mechanism is a theorem from one premise; the other is a theorem in the literature whose transcription is still owed.
...
There is a formal fact underneath this, and it is sharper than the historical pattern. RE's own two growth operators divide the same way: the stochastic one (Theorem 7) is statable in a language that contains no vocabulary for self-representation at all, while the constructive one (Theorem 8) cannot be posed without it. (That claim was first witnessed by a formal transcription later shown to be unfaithful, and was re-checked against the corrected shape rather than left standing on a dead witness; it survives, and in stronger form — Appendix M, Theorem 8's complementarity note.) The boundary between them is a boundary between *signatures*, not a difference of degree — below it, self-modeling is not merely absent but unmentionable. Escape from a boundary object is therefore never a matter of trying harder in the layer's own language; the language is the constraint.
...
Two instances are established or strongly evidenced: the Gödel sentence for arithmetic (diagonal mechanism, unconditional) and the circuit lower bounds behind P vs NP for computation (opacity mechanism, conditional on cryptographic hardness, with Razborov's bounded-arithmetic results as proof-theoretic confirmation). The formal statement, with falsifiers, is Conjecture 7 in Appendix M. Both closed cases now rest on machine-verified ground: Gödel's first incompleteness theorem was mechanised in Isabelle by Paulson (2014), and Cook–Levin fully mechanised in Coq by Gäher and Kunze (2021) — so the two encoding events this section treats as the same threshold phenomenon have each been checked by a proof assistant, independently and for unrelated reasons. The conjecture's own frame is machine-checked too, though far more weakly and in a different sense: `formal/BoundaryResidue.lean` records the representation choices and compiles, but proves nothing.
